...
Codex CLI 具备原生的插件与市场管理系统，推荐使用以下方式一键安装：

##### 方法 1：从 GitHub 仓库一键安装（推荐，全自动自举）
这是最便捷的安装方式。插件中内置了轻量级跨平台 JS 自举引导脚本，会在您首次运行 `ticket` 时自动从 GitHub Releases 下载适用于当前系统架构的预编译二进制文件：
